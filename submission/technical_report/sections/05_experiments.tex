...
\noindent\textbf{Score caveat:} The 100/100 above is the \emph{offline} objective score from our scorer aligned with the official \code{\_score\_steps} rule, computed on the current Biendata validation zip. It is \textbf{not} the public leaderboard score: the official GitHub leaderboard only lists self-reported subjective scores, and the Biendata platform score depends on the last-uploaded zip and final video/code audit. A local 100 does not imply the platform already shows 100. Physics audit on the same zip reports fail=0, warn=0, ok=5 (see \code{submission/PHYSICS\_AUDIT.md}).

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/fig5_5level_scores.png}
    \caption{Per-level official objective scores on FactorySorting after ERRATUM-aligned regen. All 5 levels achieve the maximum (L1{=}10, L2{=}15, L3{=}20, L4{=}25, L5{=}30), totaling 100/100. Bar labels show current objects/routes (L3 blue tote @ \code{aux\_input\_1}; L5 multi white tote @ \code{aux\_output\_1}).}
...
\begin{figure}[htbp]
    ...
    \caption{(a)~Final official objective per level (reported zip). (b)~Historical BC$\rightarrow$scripted debug checkpoints from an earlier campaign---single-run narrative scores, \emph{not} a claim that skip-lift alone yields 100 under current ERRATUM tasks. Interpret alongside Table~\ref{tab:ablation}: final delivery also required object-keyed poses, aux stations, contact-gated attach, sim-rebind, and nav-tuck.}
    \label{fig:ablation}
\end{figure}
