...
\begin{figure}[htbp]
    ...
    \caption{Container vs.\ tote grasp strategies. Containers (left) use a dual-arm grasp with both fingerpads clamping the rigid body, followed by a physical lift of $0.15\,\mathrm{m}$. Totes (right) have thin walls ($14\,\mathrm{mm}$) that cannot be clamped by the $46\,\mathrm{mm}$ fingerpad spacing, so a single-arm grasp with relaxed contact criterion is used, followed by skip-lift and weld-to-gripper via \code{capture\_transport\_attachment}.}
