...
class FactorySorting7_3FO3ERFKY9RN(ManipulationEnv):
    ...
    def _post_action(self, action):
        ...
        if judge_collision_pairs:
            ...
            # Keep the official collision state on every step, but avoid flooding
            # stdout with the same persistent contact pair on each control tick.
            collision_signature = tuple(judge_collision_pairs)
            if collision_signature != getattr(self, "_judge_last_reported_collision_pairs", None):
                self._print_judge_collision_info(judge_collision_pairs)
                self._judge_last_reported_collision_pairs = collision_signature
