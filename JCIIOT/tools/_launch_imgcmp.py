...
demo_img = np.asarray(f["data/demo_1/obs/robot0_robotview_image"][0], dtype=float)
f.close()
env.reset(); st = env.get_state(); obs = env.reset_to(st)
eval_img = np.asarray(obs["robot0_robotview_image"], dtype=float)
print("demo_img shape", demo_img.shape, "eval_img shape", eval_img.shape)
print("img maxabsdiff", float(np.max(np.abs(demo_img-eval_img))))
print("demo_img mean", round(float(demo_img.mean()),2), "eval_img mean", round(float(eval_img.mean()),2))
print("demo_img std", round(float(demo_img.std()),2), "eval_img std", round(float(eval_img.std()),2))
# also compare full obs dict keys present
print("eval obs keys sample:", list(obs.keys())[:8])
'''
c = d.Dswhub()
c.s.put(d.BASE + "/api/contents/_imgcmp.py", json={"type":"file","format":"text","content":INNER.replace("__CKPT__",CKPT).replace("__HDF5__",HDF5).replace("__EVFILE__",EVFILE)}, timeout=30)
print(c.run_python("import subprocess,os\nenv={**os.environ,'MUJOCO_GL':'osmesa','OMP_NUM_THREADS':'8','OPENBLAS_NUM_THREADS':'8','MKL_NUM_THREADS':'8','NUMEXPR_NUM_THREADS':'8'}\nr=subprocess.Popen(['nohup','python','/mnt/workspace/_imgcmp.py'],stdout=open('/mnt/workspace/_imgcmp.log','w'),stderr=subprocess.STDOUT,start_new_session=True,env=env)\nprint('launched',r.pid)", timeout=60))
