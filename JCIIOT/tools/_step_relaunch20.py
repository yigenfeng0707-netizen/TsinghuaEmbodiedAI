...
launch_py = (
    ...
    "cmd=['python',script,'--num-rollouts','20','--no-render','--output-name','l1_20']\n"
    "with open('/mnt/workspace/_collect20.log','w') as log:\n"
    "    r=subprocess.run(cmd,stdout=log,stderr=subprocess.STDOUT,timeout=21600,env=os.environ.copy())\n"
    "    log.write('\\nCOLLECT_RC=%d\\n'%r.returncode)\n"
    "print('collect rc',r.returncode)\n"
)
c = d.Dswhub()
c.s.put(d.BASE + "/api/contents/_collect20.py", json={"type":"file","format":"text","content":launch_py}, timeout=30)
print(c.run_python(
    "import subprocess,os\n"
    "env={**os.environ,'MUJOCO_GL':'osmesa','OMP_NUM_THREADS':'8','OPENBLAS_NUM_THREADS':'8','MKL_NUM_THREADS':'8','NUMEXPR_NUM_THREADS':'8'}\n"
    "r=subprocess.Popen(['nohup','python','/mnt/workspace/_collect20.py'],stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL,start_new_session=True,env=env)\n"
