...
demo_obs0 = {k: f["data/demo_1/obs/"+k][0] for k in f["data/demo_1/obs"].keys()}
demo_act0 = f["data/demo_1/actions"][0]
f.close()
# build obs dict as robomimic expects: nested by modality? check policy.get_action signature
import inspect
print("get_action sig:", inspect.signature(policy.get_action) if hasattr(policy,'get_action') else 'n/a')
try:
    a = policy(ob=demo_obs0)
    print("policy(ob=demo_obs0) diff vs demo_act0:", float(np.linalg.norm(np.asarray(a)-np.asarray(demo_act0))))
    print("policy out norm", float(np.linalg.norm(a)), "demo norm", float(np.linalg.norm(demo_act0)))
except Exception as e:
    print("ERR", repr(e)[:300])
# also check action_normalization_stats in ckpt
am = ckpt_dict.get("action_normalization_stats")
print("action_normalization_stats present:", am is not None)
if am:
    for k in list(am.keys())[:3]:
        print(k, {kk: (np.asarray(vv).round(3).tolist() if hasattr(vv,'tolist') else vv) for kk,vv in (am[k].items() if isinstance(am[k],dict) else [])})
'''
c = d.Dswhub()
c.s.put(d.BASE + "/api/contents/_polnet.py", json={"type":"file","format":"text","content":INNER.replace("__CKPT__",CKPT).replace("__HDF5__",HDF5).replace("__EVFILE__",EVFILE)}, timeout=30)
print(c.run_python("import subprocess,os\nenv={**os.environ,'MUJOCO_GL':'osmesa','OMP_NUM_THREADS':'8','OPENBLAS_NUM_THREADS':'8','MKL_NUM_THREADS':'8','NUMEXPR_NUM_THREADS':'8'}\nr=subprocess.Popen(['nohup','python','/mnt/workspace/_polnet.py'],stdout=open('/mnt/workspace/_polnet.log','w'),stderr=subprocess.STDOUT,start_new_session=True,env=env)\nprint('launched',r.pid)", timeout=60))
