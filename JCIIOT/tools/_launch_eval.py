...
ckpt="__CKPT__"
cmd=["python",script,"--checkpoint",ckpt,"--num-rollouts","1","--no-render","--device","cpu"]
# detect factory scene default
with open(script) as f: s=f.read()
import re
m=re.search(r'DEFAULT_FACTORY_SCENE\s*=\s*["\']([^"\']+)', s)
print("DEFAULT_FACTORY_SCENE=", m.group(1) if m else None)
if m: cmd += ["--factory-scene", m.group(1)]
