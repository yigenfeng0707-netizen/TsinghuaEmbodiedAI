...
ckpt="__CKPT__"
scene="__SCENE__"
cmd=["python", script, "--checkpoint", ckpt, "--factory-scene", scene, "--num-rollouts", "3", "--no-render", "--device", "cpu"]
with open("/mnt/workspace/_eval.log","w") as log:
    ...
    log.write("\\nEVAL_RC=")
    log.write(str(r.returncode))
    log.write("\\n")
print("eval rc", r.returncode)
'''
INNER = INNER.replace("__CKPT__", CKPT).replace("__SCENE__", SCENE)
