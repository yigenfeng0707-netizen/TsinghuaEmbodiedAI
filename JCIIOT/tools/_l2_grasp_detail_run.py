...
# run scripted grasp via the integrated function
result = RB._scripted_grasp_in_wrapped_env(wrapped, raw, obj_name, headless=True, render_callback=None)
log("scripted grasp result:", result)

# detailed: check eef vs target after grasp
below_targets,_=col.get_target_positions(raw, obj_name, 0.035)
for arm in col.ARMS:
    ep=col.get_eef_pos(raw, robot, arm)
    log(f"  eef {arm}: {ep.tolist()} target {below_targets[arm].tolist()} dist {np.linalg.norm(ep-below_targets[arm]):.3f}")
gs=grasp_status(raw, robot, obj_name)
log("grasp_status:", gs)
backend.close()
log("DONE")
'''
...
c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_l2_grasp_detail.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_l2_grasp_detail.py'],stdout=open('/mnt/workspace/_l2_grasp_detail.log','w'),stderr=subprocess.STDOUT,env=env)\n"
