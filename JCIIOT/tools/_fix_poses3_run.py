...
# Compute correct base pose per LEVEL using object grasp site position.
# L1 reference offset: base = grasp_mid + [0.651, -0.019] (container, worked)
# For tote (higher) use larger offset 1.0-1.2 to avoid start-in-object.
level_offset = {"L1":0.65, "L2":1.0, "L3":1.2, "L4":0.65, "L5":1.2}
poses_by_level = {}
for lvl in ["L1","L2","L3","L4","L5"]:
    task=next(t for t in cfg["tasks"] if t["level"]==lvl)
    obj_name=task["object"]
    # use existing source pose yaw (all -3.14)
    yaw = -3.14
    # need a base pose to load env; use a placeholder then read site
    # Actually site positions are world-frame and independent of base. Use any base.
    ...
    backend.reset()
    # build wrapped env with placeholder base to read sites
    ...
    new_base = grasp_mid + np.array([off, -0.019])
    poses_by_level[lvl] = {"pos":[round(new_base[0],3),round(new_base[1],3),0.0], "yaw":yaw}
    log(f"{lvl}: grasp_mid={grasp_mid.tolist()} off={off} -> base={new_base.tolist()}")
    backend.close()

cfg["grasp_poses_by_level"] = poses_by_level
json.dump(cfg, open(P(APP)/"knowledge"/"task_config.json","w"), indent=2, ensure_ascii=False)
log("added grasp_poses_by_level to task_config.json")
log("DONE")
'''

c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_fix_poses3.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_fix_poses3.py'],stdout=open('/mnt/workspace/_fix_poses3.log','w'),stderr=subprocess.STDOUT,env=env)\n"
