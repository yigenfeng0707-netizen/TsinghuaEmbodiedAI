...
# test offset 0.45 with arm pre-lift
for off in [0.45, 0.5, 0.55]:
    base_pos=[grasp_mid[0]+off, grasp_mid[1], 0.0]
    backend=RobosuiteBackend(env_name=task["env_name"],camera="birdview",headless=True,drive_mode="direct")
    backend.set_physics_grasp_config(device="cpu",object_map=objs)
    backend.reset()
    ...
    log(f"  off={off}: touch={touch} R_dist={rd:.3f} L_dist={ld:.3f}")
    backend.close()
log("DONE")
'''

c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_near_test.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_near_test.py'],stdout=open('/mnt/workspace/_near_test.log','w'),stderr=subprocess.STDOUT,env=env)\n"
