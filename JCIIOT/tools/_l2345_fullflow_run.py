...
for lvl in ["L2","L3","L4","L5"]:
    ...
    log(f"\n===== {lvl}: {task['env_name']} obj={task['object']} =====")
    try:
        prefix_map = {"L2":"factory_sorting_3_3fo3errph7x9","L3":"factory_sorting_5_3fo3ertpxeut","L4":"factory_sorting_7_3fo3erfky9rn","L5":"factory_sorting_9_3fo3ert2c5fp"}
        ...
        for s in report.steps:
            log(f"    {s.skill_name}: {'OK' if s.success else 'FAIL'} {(s.message or '')[:60]}")
        # final object pos
        env = backend.env
        for jn in env.sim.model.joint_names:
            if task["object"] in jn and ("_free" in jn or "_joint0" in jn):
                ...
                log(f"  final object pos: {qpos.tolist()[:3]}")
                break
        backend.close()
    except Exception as e:
        log(f"  {lvl} FAIL: {repr(e)[:200]}")
        log(_tb.format_exc()[-1000:])
log("\nALL DONE")
'''

c = d.Dswhub()
c.s.put(d.BASE+"/api/contents/_l2345_fullflow.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa','GATE_OLLAMA':'false'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_l2345_fullflow.py'],stdout=open('/mnt/workspace/_l2345_fullflow.log','w'),stderr=subprocess.STDOUT,env=env)\n"
