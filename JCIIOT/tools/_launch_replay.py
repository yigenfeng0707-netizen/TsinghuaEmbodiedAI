...
INNER = '''import os, sys, h5py, numpy as np
os.environ["MUJOCO_GL"]="osmesa"
sys.path.insert(0, "/mnt/workspace/JCIIOT_repo/JCIIOT")
mod = __import__("robosuite.robosuite.environments.factory_sorting.load_factory_sorting_evalization", fromlist=["*"])
args = __import__("argparse").Namespace(
    checkpoint=__import__("pathlib").Path("__CKPT__"),
    ...
    object_site_size=0.02, robot_base_pos=None, robot_base_ori=None,
    renderer="mjviewer", camera="robot0_robot", controller=None,
...
acts = f["data/demo_1/actions"][:]
f.close()
print("demo_1 actions shape:", acts.shape, "action_dim env:", env.action_dimension)
env.reset(); st = env.get_state(); obs = env.reset_to(st)
raw_env = mod.base_robosuite_env(env)
robot = raw_env.robots[0]
obj = args.object_name
targets = mod.print_reset_debug_info(raw_env, obj, args)
policy.start_episode()
na = min(len(acts), env.action_dimension and len(acts))
success=False
...
'''.replace("__CKPT__", CKPT).replace("__SCENE__", SCENE).replace("__HDF5__", HDF5)
