...
backend.reset(); log("reset2 ok MUJOCO_GL=",os.environ.get("MUJOCO_GL")," PYOPENGL=",os.environ.get("PYOPENGL_PLATFORM"))
log("glfw module file:", getattr(__import__("glfw"),"__file__","?"))
# wrap glfw.init robustly: also patch any already-imported reference
import glfw
_orig=glfw.init
def _wi(*a,**k):
    log("[glfw.init WRAPPED]", "".join(_tb.format_stack()[-8:]))
    return _orig(*a,**k)
glfw.init=_wi
import sys as _s
for m in list(sys.modules.values()):
    if m is None: continue
    if hasattr(m,"init") and getattr(m,"init",None) is _orig:
        try: m.init=_wi
...
c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_bisect11.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa','GATE_OLLAMA':'false'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_bisect11.py'],stdout=open('/mnt/workspace/_bisect11.log','w'),stderr=subprocess.STDOUT,env=env)\n"
