...
backend.close()
log(f"L3 grasp_mid={grasp_mid.tolist()} site_R={[rsite[0],rsite[1]]} site_L={[lsite[0],lsite[1]]}")

# test y offsets with x offset 0.8
for yoff in [-0.1, -0.05, 0.0, 0.05, 0.1]:
    base_pos=[grasp_mid[0]+0.8, grasp_mid[1]+yoff, 0.0]
    backend=RobosuiteBackend(env_name=task["env_name"],camera="birdview",headless=True,drive_mode="direct")
    backend.set_physics_grasp_config(device="cpu",object_map=objs)
    backend.reset()
    ...
    rd=np.linalg.norm(starts["right"][:2]-below_targets["right"][:2])
    ld=np.linalg.norm(starts["left"][:2]-below_targets["left"][:2])
    log(f"  yoff={yoff}: touch={touch} R_dist={rd:.3f} L_dist={ld:.3f}")
    backend.close()
log("DONE")
'''

c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_yoff_test.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_yoff_test.py'],stdout=open('/mnt/workspace/_yoff_test.log','w'),stderr=subprocess.STDOUT,env=env)\n"
