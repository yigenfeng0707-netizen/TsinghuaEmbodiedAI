...
for lvl in ["L3","L5"]:
    ...
    # check if eef is inside object
    obj_geoms = col.object_collision_geoms(raw, obj_name)
    log(f"  starts: { {k:v.tolist() for k,v in starts.items()} }")
    log(f"  below_targets: { {k:v.tolist() for k,v in below_targets.items()} }")
    log(f"  obj_geoms: {obj_geoms}")
    touch = col.gripper_touches_object(raw, robot, obj_name)
    log(f"  gripper_touches_object at start: {touch}")
    # distance eef to object
    for arm in col.ARMS:
        ep = col.get_eef_pos(raw, robot, arm)
        bt = below_targets[arm]
        log(f"  {arm} eef-to-target dist: {np.linalg.norm(ep-bt):.3f}")
    backend.close()
log("DONE")
'''

c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_l35_diag.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_l35_diag.py'],stdout=open('/mnt/workspace/_l35_diag.log','w'),stderr=subprocess.STDOUT,env=env)\n"
