...
# L1 reference: base offset [0.65, -0.019] from grasp_mid → symmetric arms
# Tote: use [0.45, -0.019] (same y offset as L1 for arm symmetry)
...
for lvl in ["L1","L2","L3","L4","L5"]:
    ...
    # same y offset as L1 (-0.019) for arm symmetry
    new_base = grasp_mid + np.array([off, -0.019])
    poses_by_level[lvl] = {"pos":[round(new_base[0],3),round(new_base[1],3),0.0], "yaw":yaw}
    log(f"{lvl}: grasp_mid={grasp_mid.tolist()} off=[{off},-0.019] -> base={new_base.tolist()}")
    backend.close()
...
c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_fix_poses6.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_fix_poses6.py'],stdout=open('/mnt/workspace/_fix_poses6.log','w'),stderr=subprocess.STDOUT,env=env)\n"
