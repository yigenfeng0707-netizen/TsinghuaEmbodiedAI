...
# CASE A: no set_physics_grasp_config, reset twice, then follow_path
...
backend.reset(); log("reset2 ok (no physics)")
xy,yaw=backend.get_base_pose()
try:
    res=backend.follow_path([_np.array(xy)], max_steps=50, record_every=0)
    log("A follow_path res", res)
except Exception as e:
    log("A follow_path FAIL", repr(e))
backend.close()
log("CASE A DONE")
'''

c=d.Dswhub()
c.s.put(d.BASE+"/api/contents/_bisect7.py", json={"type":"file","format":"text","content":INNER}, timeout=30)
print(c.run_python(
"import subprocess,os\n"
"env={**os.environ,'MUJOCO_GL':'osmesa','PYOPENGL_PLATFORM':'osmesa','GATE_OLLAMA':'false'}\n"
"p=subprocess.Popen(['python','/mnt/workspace/_bisect7.py'],stdout=open('/mnt/workspace/_bisect7.log','w'),stderr=subprocess.STDOUT,env=env)\n"
