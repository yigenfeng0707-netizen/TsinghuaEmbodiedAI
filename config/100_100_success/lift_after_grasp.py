...
def lift_grasped_object(
    ...
    # Stage 255: tote 物体 left arm 无法接触 wall（几何限制），right arm 已稳定抓住
    # 对 tote 使用 any()，其他物体保持 all()
    _is_tote = "tote" in object_name.lower()
    _grasp_ok_initial = any(initial_grasp_status.values()) if _is_tote else all(initial_grasp_status.values())
    if not _grasp_ok_initial:
        return {
            "success": False,
            "failure_reason": f"object is not grasped by {'any' if _is_tote else 'both'} grippers: {initial_grasp_status}",
        }
    ...
    # Stage 255: tote 物体使用 any() 检查 final grasp
    _grasp_ok_final = any(final_grasp_status.values()) if _is_tote else all(final_grasp_status.values())
    if not _grasp_ok_final:
        return {
            "success": False,
